...
public class YarsPGReader {
    ...
    private final Map<String, String> cache = new HashMap<>(1024 * 32);
    ...
    public void nodeFetch(String nodeId, List<Label> labels,
        ...
        cache.put(nodeId, node.getElementId());
    ...
    public void createRelationship(String leftNodeId, RelationshipType relationshipType, String rightNodeId,
                                   Map<String, Object> properties) {
        tx.increment();
        Node nodeFrom = tx.getTransaction().getNodeByElementId(cache.get(leftNodeId));
        Node nodeTo = tx.getTransaction().getNodeByElementId(cache.get(rightNodeId));
    ...
    private void handleLackingNodes() {
        ...
        lackingNodes.forEach((k, v) -> {
            if (cache.containsKey(v.leftNodeId()) && cache.containsKey(v.rightNodeId())) {
                createRelationship(v.leftNodeId(), v.relationshipType(), v.rightNodeId(), v.properties());
            }
        });
    }
